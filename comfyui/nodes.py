...
v1 design: each task node shells out to inference_lance.py per call. Slow
(~30-60 s per generation due to model reload) but correct, robust, and avoids
having to refactor Lance's internals. v2 will hold the model resident.
...
# ---------------------------------------------------------------------------
# Subprocess runner — shells out to scripts/run_baseline.py or run_quant_eval.py
# ---------------------------------------------------------------------------
...
def _run_lance_cli(*, task: str, model_path: str, vit_path: str,
    ...
    if awq_dir:
        ...
        cmd = [py, str(src / script), "--task", task,
                "--model_path", model_path, "--vit_path", vit_path,
                "--awq_dir", awq_dir, "--save_path_gen", save_dir,
    ...
    t0 = time.time()
    res = subprocess.run(cmd, cwd=str(src), env=env, capture_output=True, text=True)
...
class LanceModelLoader:
    ...
    def load(self, flavor, precision):
        root = _model_root()
